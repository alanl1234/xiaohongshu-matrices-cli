...
def _prune_token_cache(
    ...
    pruned = OrderedDict(
        (key, value) for key, value in cache.items() if now - float(value.get("ts", 0)) <= NOTE_CONTEXT_TTL_SECONDS
    )
...
def load_token_cache() -> dict[str, dict[str, Any]]:
    ...
    with _TOKEN_CACHE_LOCK:
        ...
        return {key: dict(value) for key, value in _TOKEN_CACHE_MEMORY.items()}
...
def save_token_cache(cache: dict[str, dict[str, Any]]) -> None:
    ...
    normalized = _prune_token_cache(
        OrderedDict(
            sorted(
                ((str(key), dict(value)) for key, value in cache.items() if key and isinstance(value, dict)),
...
def save_note_index(items: list[dict[str, str]]) -> None:
    ...
    normalized = [entry for entry in (_normalize_index_entry(item) for item in items) if entry]
...
def _get_browser_loader(source: str):
    ...
    if loader is None or not callable(loader):
        ...
        raise ValueError(f"Unknown browser: {source!r}. Available: {', '.join(available)}")
    return loader
...
def get_cookies(cookie_source: str = "auto", *, force_refresh: bool = False) -> tuple[str, dict[str, str]]:
    """
