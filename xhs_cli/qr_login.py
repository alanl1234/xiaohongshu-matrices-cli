...
def _build_saved_cookies(a1: str, webid: str, payload: dict[str, Any]) -> dict[str, str]:
    ...
    secure_session = (
        payload.get("secure_session") or login_info.get("secure_session") or payload.get("web_session_sec", "")
    )
    cookies = {
        "a1": a1,
        "webId": webid,
...
def _raise_for_browser_response(response: Any) -> None:
    ...
    if status and status >= 400:
        try:
            ...
            body = "<unavailable>"
        raise XhsApiError(f"Browser-assisted QR completion failed: HTTP {status} body={body[:300]}")
...
def _ensure_camoufox_ready() -> None:
    ...
    try:
        ...
        raise BrowserQrLoginUnavailable("Browser-assisted QR login requires the `camoufox` package.") from exc
    ...
    try:
        ...
        raise BrowserQrLoginUnavailable("Unable to validate the Camoufox browser installation.") from exc

    if result.returncode != 0 or not result.stdout.strip():
        raise BrowserQrLoginUnavailable("Camoufox browser runtime is missing. Run `python -m camoufox fetch` first.")
...
def _browser_assisted_qrcode_login(
    *,
    ...
    try:
        ...
        raise BrowserQrLoginUnavailable("Camoufox sync API is unavailable in the current environment.") from exc
